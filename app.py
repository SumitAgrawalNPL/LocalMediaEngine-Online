    ...
    if st.button("RUN ENGINE OPERATIONAL ARRAY", type="primary"):
        if not uploaded_file:
            ...
            try:
                ...
                st.error(f"FFmpeg Matrix Operational Fault: {import streamlit as st
import os
import sys
import subprocess
import shutil
        ...
        if not url:
            ...
            with st.spinner("Acquiring container headers and streaming chunks..."):
                opts = {
                    'outtmpl': os.path.join(out_dir, '%(title)s.%(ext)s'),
                    'noplaylist': True,
                    # Impersonate authentic modern browser requests to mask data-center identifiers
                    'http_headers': {
                ...
                # Automatically map server session tokens if cookies are present
                if os.path.exists(COOKIES_FILE): 
                    opts['cookiefile'] = COOKIES_FILE
                
                # Updated Format Configurations to bypass fragment blockades
                ...
                try:
                    ...
                    st.markdown("""
                    ### 🛠️ 100% Permanently Bypassing Multi-Fragment Blocks
                    If specific regional videos or restricted media streams still flag a 403 block on your cloud node, it means YouTube is requiring direct authentication signatures. You can completely bypass this limit:
                    1. Install a free browser extension like **Get cookies.txt LOCALLY** on your desktop web browser.
                    2. Navigate to YouTube, open the extension panel, and hit **Export**.
                    3. Save that file exactly as **`cookies.txt`** and drag it directly into your GitHub project repository. Your app will automatically sign into downloads and completely eliminate all HTTP restrictions.
                    """)
